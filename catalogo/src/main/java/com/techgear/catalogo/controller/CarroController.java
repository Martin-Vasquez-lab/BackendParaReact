...
public class CarroController {
    ...
    public ResponseEntity<Void> eliminarCarro(@PathVariable("id")Integer id){
        ...
        @PostMapping("/batch")
    @Operation(summary="Crear múltiples items del carrito",description = "Crea varios registros del carrito en lote para sincronización")
    @ApiResponse(responseCode = "200", description = "Items del carrito creados",
        content = @Content(array = @ArraySchema(schema = @Schema(implementation = Carro.class)))
    )
    public ResponseEntity<List<Carro>> createCarroBatch(@RequestBody List<Carro> carros) {
        try {
            List<Carro> savedCarros = carroService.saveAll(carros);
            return ResponseEntity.ok(savedCarros);
        } catch (Exception e) {
            return ResponseEntity.internalServerError().build();
        }
    }
